...
KISSY.add(function (S, D, E, Base) {
    ...
    var loadImgSrc = function (img, flag, onStart, webpReplacer) {
      ...
      //  console.log('result && param.src=', result && param.src);
    ...
    function Lazyload(container, config) {
        ...
        if (!(self instanceof Lazyload)) {
            return new Lazyload(container, config);
        }
        ...
        Lazyload.superclass.constructor.call(self, newConfig);

        if(newConfig.cacheSize === false){
        	cacheSize = false;
        }
        // 和延迟项绑定的回调函数
        self._callbacks = {};
        ...
        self['_initLoadEvent']();
		newConfig.container && self.addElements(newConfig.container, self.get('type'));
        self._loadFn();
        S.ready(function () {
            self._loadFn();
        });
    ...
    Lazyload.ATTRS = {
	     ...
	     */
	    placeholder: {
	        value: '../resources/images/default_product.png'
	    },
    ...
    S.extend(Lazyload, Base, {
        ...
        * @param {function(this: HTMLElement): boolean} outfn  
        * @Callback function to be called when el is outof viewport.
        * return false to indicate el is actually not in viewport( for example display none element ).
        */
       addCallback: function (el, fn, outfn) {
           el = D.get(el);
        ...
        removeCallback: function (el, fn) {
   	 	...
   	 	imgHandle : function (el) {
   	 		loadImgSrc(el, this.get('imgFlag'), this.get('onStart'));
